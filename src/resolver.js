...
import { invidiousChannel, invidiousSearch, invidiousTrending, invidiousWatch } from './invidious.js';
import { channelFallback, searchFallback, trendingFallback, watchFallback } from './ytdlp.js';
...
export async function resolveSearch(query) {
  ...
  return getOrSetCachedJson(key, CACHE_TTL_MS, async () => {
    ...
    try {
      ...
      return {
        source: videos?.source || 'yt-dlp',
        results: (videos?.videos || []).map((item) => normalizeSearchResult(item, 'yt-dlp')),
        fallbackErrors
      };
    } catch (error) {
...
export async function resolveWatch(videoId) {
  ...
  return getOrSetCachedJson(key, CACHE_TTL_MS, async () => {
    ...
    try {
      ...
      if (proxied) {
        return { ...normalizeWatchResult(proxied, 'yt-dlp-proxy'), fallbackErrors, playbackSource: 'yt-dlp-proxy' };
      }
    } catch (error) {
    ...
    try {
      ...
      if (direct) {
        return { ...normalizeWatchResult(direct, 'yt-dlp-direct'), fallbackErrors, playbackSource: 'yt-dlp-direct' };
      }
...
export async function resolveChannel(channelId, continuation = '') {
  const safeId = String(channelId || '').trim();
  const key = `channel:${safeId}:${continuation}`;
  return getOrSetCachedJson(key, CACHE_TTL_MS, async () => {
    const fallbackErrors = [];

    try {
      const fromInvidious = await invidiousChannel(safeId, continuation);
      return {
        source: 'invidious',
        meta: fromInvidious.meta,
        videos: fromInvidious.videos.map((item) => normalizeSearchResult(item, 'invidious')),
        continuation: fromInvidious.continuation || '',
        fallbackErrors
      };
    } catch (error) {
      fallbackErrors.push({ source: 'invidious', error: error.message });
      log.warn('Invidious channel failed', { channelId: safeId, error: error.message });
    }

    try {
      const direct = await channelFallback(safeId, { useProxy: false });
      if (direct) {
        return {
          source: 'yt-dlp',
          meta: direct.meta,
          videos: (direct.videos || []).map((item) => normalizeSearchResult(item, 'yt-dlp')),
          continuation: '',
          fallbackErrors
        };
      }
      ...
      return {
        source: 'error',
        meta: { id: safeId, title: 'Channel unavailable' },
        videos: [],
        continuation: '',
        error: error.message,
        fallbackErrors
