...
function normalizeFormats(item) {
  ...
  if (hlsUrl) {
    out.push({ id: 'hls', label: 'HLS', directUrl: hlsUrl, mimeType: 'application/vnd.apple.mpegurl', hasAudio: true, hasVideo: true, hls: true, source: 'invidious', bandwidth: 0, width: 0, height: 0, fps: 0, ext: 'm3u8', audioCodec: 'unknown', videoCodec: 'unknown', container: 'm3u8', note: 'direct-hls' });
  }
  return out;
}
...
function normalizeVideo(data) {
  ...
  for (const format of adaptive) {
    ...
    formats.push({
      ...
      label: labelFormat({ height: format.height || Number.parseInt(String(format.qualityLabel || format.resolution || ''), 10) || 0, fps: format.fps || 0, audioCodec: format.audioQuality || format.audioChannels || 'unknown', videoCodec: format.container || format.ext || 'unknown', mimeType: format.type || format.mimeType || '', id: format.itag || format.quality || format.id }),
      directUrl: url,
  ...
  if (hlsUrl) {
    formats.unshift({ id: 'hls', label: 'HLS', directUrl: hlsUrl, mimeType: 'application/vnd.apple.mpegurl', hasAudio: true, hasVideo: true, hls: true, source: 'invidious', bandwidth: 0, width: 0, height: 0, fps: 0, ext: 'm3u8', audioCodec: 'unknown', videoCodec: 'unknown', container: 'm3u8', note: 'direct-hls' });
  }
...
export async function invidiousSearch(query) {
  ...
  const { data } = await firstSuccessfulJson(path, { timeoutMs: REQUEST_TIMEOUT_MS, validate: (payload) => (Array.isArray(payload) ? payload.map(normalizeSearchItem).filter(Boolean) : []) });
  return data;
}
...
export async function invidiousWatch(videoId) {
  const { data } = await firstSuccessfulJson(`/videos/${encodeURIComponent(videoId)}?hl=en`, { timeoutMs: REQUEST_TIMEOUT_MS, validate: (payload) => payload && typeof payload === 'object' && (payload.videoId || payload.id) });
